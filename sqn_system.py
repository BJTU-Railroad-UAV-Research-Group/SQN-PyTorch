...
from dataset.Env_dataset import Env, EnvSampler
...
class SQN_System(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        if self.hparams['hparams'].dataset_name == 'S3DIS' or self.hparams['hparams'].dataset_name == 'Semantic3D' or self.hparams['hparams'].dataset_name == 'Env':
    ...
    def setup(self, stage):
        if self.hparams['hparams'].work_type == 'train':
            ...
            elif self.hparams['hparams'].dataset_name == 'Env':
                dataset = Env()
                self.training_dataset = EnvSampler(dataset, 'training')
                self.validation_dataset = EnvSampler(dataset, 'validation')
            ...
            elif self.hparams['haparams'].dataset_name == 'Env':
                dataset = Env()
                self.test_dataset = EnvSampler(dataset, 'validation')
        else:
    ...
    def on_train_epoch_end(self):
        ...
        self.log('train/mean_iou', mean_iou, sync_dist=True)
...
def train(args):
    ...
    wandb_logger = WandbLogger(project="SQN", name=args.name, save_dir="/data1/mengfanteng/SQN_exp/Env")
