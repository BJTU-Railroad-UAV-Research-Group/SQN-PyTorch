...
from dataset.Env_dataset import Env, EnvSampler # when training with xyz + intensity
...
parser.add_argument('--checkpoint_path', default='/data1/mengfanteng/SQN_exp/Env/2024-12-27_08-54-01/checkpoint.tar', help='Model checkpoint path [default: None]')
parser.add_argument('--log_dir', default='/data1/mengfanteng/SQN_exp/Env/2024-12-27_08-54-01/test_output', help='Dump dir to save model checkpoint [default: log]')
...
test_dataloader = DataLoader(test_dataset, batch_size=cfg.val_batch_size, shuffle=True, collate_fn=test_dataset.collate_fn, pin_memory=True)
    ...
    def test(self, dataset, num_vote=10):
        ...
        while last_min < num_vote:
            ...
            for batch_idx, batch_data in enumerate(test_dataloader):
                ...
                # Forward pass
                with torch.no_grad():
                    end_points = net(batch_data, is_training=False)
                ...
                if stacked_probs.shape[0] != cfg.num_points:
                    continue
                stacked_probs = torch.reshape(stacked_probs, [cfg.val_batch_size, cfg.num_points,
                                            cfg.num_classes])
                stacked_probs = F.softmax(stacked_probs, dim=-1).cpu().numpy()
